...
ridge_plot <- function(df){
  ...
  stat <- df %>% pairwise_t_test(correlation~Feature, p.adjust.method='bonferroni') %>% filter((group1=='ema' & group2=='pca-residuals') | (group1=='sfa-ema' & group2=='residuals')) %>% add_xy_position(x="Feature") %>% mutate(y.position=y.position-0.1)
...
ridge <- ridge %>% mutate(correlation=values, Feature=factor(feat1, levels=c("ema", "pca-residuals", "pca-wav", "sfa-ema", "residuals", "wavlm"))) 
### STATS
...
clf_plot <- function(df){
  ...
  p_list <- list()
  acc_stat <- acc_df %>% pairwise_t_test(accuracy~Feature, p.adjust.method='bonferroni')  %>% filter((group1=='ema' & group2=='pca-residuals') | (group1=='sfa-ema' & group2=='residuals')) %>% add_xy_position(x = "Feature")
  ...
  bacc_stat <- bacc_df %>% pairwise_t_test(balanced_accuracy~Feature, p.adjust.method='bonferroni') %>% filter((group1=='ema' & group2=='pca-residuals') | (group1=='sfa-ema' & group2=='residuals')) %>% add_xy_position(x="Feature")
...
mlt$feat1[mlt$feat1== 'wavlm-large.8'] = 'wavlm'
mlt$feat2[mlt$feat2== 'word'] = 'Word Identity'
mlt$feat2[mlt$feat2== 'phone'] = 'Phone Identity'
mlt <- mlt %>% mutate(Feature=factor(feat1, levels=c("ema", "pca-residuals", "pca-wav","sfa-ema","residuals", "wavlm"))) 
...
mlt <- mlt %>% mutate(Feature=factor(feat1, levels=c("ema", "pca-residuals", "pca-wav", "sfa-ema", "residuals", "wavlm"))) %>% mutate(metric=metric_type)
art <- clf_plot(mlt)
