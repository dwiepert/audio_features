...
# THIS WILL FAIL IF YOU HAVEN'T RUN fix_multilabel_results.py
...
ridge_plot <- function(df){
  ...
  stat <- df %>% pairwise_t_test(correlation~Feature, p.adjust.method='bonferroni') %>% filter((group1=='ema' & group2=='pca-residuals') | (group1=='ema' & group2=='residuals')) %>% add_xy_position(x="Feature") %>% mutate(y.position=y.position-0.1)
...
ridge$feat1[ridge$feat1== 'wavlm-large.8'] = 'wavlm'
...
ridge <- ridge %>% mutate(correlation=values, Feature=factor(feat1, levels=c("ema", "pca-residuals", "residuals", "wavlm"))) 
### STATS
