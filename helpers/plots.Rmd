...
clf_plot <- function(df){
  ...
  p_list <- list()
  acc_stat <- acc_df %>% pairwise_t_test(accuracy~Feature, p.adjust.method='bonferroni')  %>% filter((group1=='ema' & group2=='pca-residuals') | (group1=='ema' & group2=='residuals')) %>% add_xy_position(x = "Feature")
  ...
  bacc_stat <- bacc_df %>% pairwise_t_test(balanced_accuracy~Feature, p.adjust.method='bonferroni') %>% filter((group1=='ema' & group2=='pca-residuals') | (group1=='ema' & group2=='residuals')) %>% add_xy_position(x="Feature")
...
mlt <- mlt %>% mutate(Feature=factor(feat1, levels=c("ema", "pca-residuals", "residuals", "wavlm"))) 
...
mlt <- mlt %>% mutate(Feature=factor(feat1, levels=c("ema", "pca-residuals", "residuals", "wavlm"))) %>% mutate(metric=metric_type)
art <- clf_plot(mlt)
